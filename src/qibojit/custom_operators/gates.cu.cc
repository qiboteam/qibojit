...
__global__ void apply_three_qubit_gate_kernel(T* state,
  ...
  // Iterating on the 8 output elements indexed by t
  for(auto i = 0; i < 8; i++) {
    ...
    state[t] = gate[i*8] * buffer0 + gate[i*8+1] * buffer1 + gate[i*8+2] * buffer2 + gate[i*8+3] * buffer3 + gate[i*8+4] * buffer4 + gate[i*8+5] * buffer5 + gate[i*8+6] * buffer6 + gate[i*8+7] * buffer7;
  } 
}
...
__global__ void apply_four_qubit_gate_kernel(T* state,
  ...
  // Iterating on the 16 output elements indexed by t
  for(auto i = 0; i < 16; i++) {
    // Get the index where the state vector must be updated
    const long t = ig - multitarget_index(targets, 15 - i, ntargets);

    // Update the state vector at index t
    state[t] = gate[i*16] * buffer0 + gate[i*16+1] * buffer1 + gate[i*16+2] * buffer2 + gate[i*16+3] * buffer3 + gate[i*16+4] * buffer4 + gate[i*16+5] * buffer5 + gate[i*16+6] * buffer6 + gate[i*16+7] * buffer7 + gate[i*16+8] * buffer8 + gate[i*16+9] * buffer9 + gate[i*16+10] * buffer10 + gate[i*16+11] * buffer11 + gate[i*16+12] * buffer12 + gate[i*16+13] * buffer13 + gate[i*16+14] * buffer14 + gate[i*16+15] * buffer15;
  }
}
...
__global__ void apply_five_qubit_gate_kernel(T* state,
  ...
  // Iterating on the 32 output elements indexed by t
  for(auto i = 0; i < 32; i++) {
    // Get the index where the state vector must be updated
    const long t = ig - multitarget_index(targets, 31 - i, ntargets);

    // Update the state vector at index t
    state[t] = gate[i*32] * buffer0 + gate[i*32+1] * buffer1 + gate[i*32+2] * buffer2 + gate[i*32+3] * buffer3 + gate[i*32+4] * buffer4 + gate[i*32+5] * buffer5 + gate[i*32+6] * buffer6 + gate[i*32+7] * buffer7 + gate[i*32+8] * buffer8 + gate[i*32+9] * buffer9 + gate[i*32+10] * buffer10 + gate[i*32+11] * buffer11 + gate[i*32+12] * buffer12 + gate[i*32+13] * buffer13 + gate[i*32+14] * buffer14 + gate[i*32+15] * buffer15 + gate[i*32+16] * buffer16 + gate[i*32+17] * buffer17 + gate[i*32+18] * buffer18 + gate[i*32+19] * buffer19 + gate[i*32+20] * buffer20 + gate[i*32+21] * buffer21 + gate[i*32+22] * buffer22 + gate[i*32+23] * buffer23 + gate[i*32+24] * buffer24 + gate[i*32+25] * buffer25 + gate[i*32+26] * buffer26 + gate[i*32+27] * buffer27 + gate[i*32+28] * buffer28 + gate[i*32+29] * buffer29 + gate[i*32+30] * buffer30 + gate[i*32+31] * buffer31;
  }
}

template<typename T>
__device__ void _apply_multi_qubit(T& state, const T* buffer, const T* gate,
                                   const long* targets, long nsubstates, int ntargets, long ig) {
  state = T(0., 0.);
  for (auto i = 0; i < nsubstates; i++) {
    for (auto j = 0; j < nsubstates; j++) {
      const long t = ig - multitarget_index(targets, nsubstates - j - 1, ntargets);
      state = cadd(state, cmult(gate[nsubstates * i + j], buffer[t]));
    }
  }
}
